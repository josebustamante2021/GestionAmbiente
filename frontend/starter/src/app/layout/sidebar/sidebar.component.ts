...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.routerObj = this.router.events.subscribe((event) => {
      if (event instanceof NavigationEnd) {
        ...
        const role = ['admin', 'teacher', 'student','role_admin'];
  ...
  ngOnInit() {
    if (this.authService.currentUserValue) {
      ...
      const res = ROUTES.filter((f) => roles.includes(f.role[0]) || f.role.indexOf('All')!== -1);
      this.userType=this.authService.currentUserValue.userPrincipalName;
